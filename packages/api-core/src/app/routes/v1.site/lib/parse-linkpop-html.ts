...
export async function parseLinkpopHtml(html: string): Promise<TLinkPopData> {
	let foundLinkpopData: string | null = null;
	...
	try {
		tokenize(
			...
			{ ...htmlConfig, contextSliceSize: 250 }
		);
	} catch (e) {
	...
	const decodedData = (foundLinkpopData as string)
		.replace(/&quot;/g, '"')
		.replace(/&amp;/g, '&')
		.replace(/&lt;/g, '<')
		.replace(/&gt;/g, '>')

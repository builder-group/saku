...
export async function verifyShopifyWebhook(c: Context): Promise<TShopifyWebhookMetadata> {
	const metadata = extractShopifyWebhookMetadata(c);

	const rawBody = c.get('rawRequestBody') as TRawRequestBody;
	if (rawBody == null) {
		throw new AppError('#ERR_NO_RAW_BODY', 500, {
 ...
 */
function extractShopifyWebhookMetadata(c: Context): TShopifyWebhookMetadata {

...
interface TLinkBlockProps {
	blockState: TState<TLinkBlock, []>;
}

export const StaticLinkBlock: React.FC<TStaticLinkBlockProps> = (props) => {
	const { block } = props;

	// Get title from custom meta or fallback to regular meta
	const getTitle = React.useCallback((): string | undefined => {
		return block.customMeta?.title ?? block.meta?.title;
	}, [block.customMeta, block.meta]);

	// Get favicon from custom meta or fallback to regular meta
	const getFaviconUrl = React.useCallback((): string | undefined => {
		return block.customMeta?.faviconUrl ?? block.meta?.faviconUrl;
	}, [block.customMeta, block.meta]);
...
interface TStaticLinkBlockProps {
	block: TLinkBlock;
}

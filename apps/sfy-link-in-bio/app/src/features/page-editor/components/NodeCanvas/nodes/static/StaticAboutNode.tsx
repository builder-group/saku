...
export const StaticAboutNode: React.FC<TStaticAboutNodeProps> = (props) => {
	const { node } = props;
			...
			<div className="relative overflow-hidden rounded-3xl bg-white shadow-sm">
				...
				<div className="flex flex-col gap-3 p-5">
					...
					<div className="flex flex-col gap-3 text-center">
						<h3 className="text-xl font-medium">{node.name}</h3>

						{node.bio && (
							<div className="line-clamp-6">
								<p className="text-[#666]">{node.bio}</p>
							</div>
						)}
					</div>
				</div>
...
interface TStaticAboutNodeProps {
	node: TAboutNode;
}
